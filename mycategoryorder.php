...
function mycategoryorder()
{
	...
	if(!is_numeric($parentID))
		return '<div id="message" class="error fade"><p>'. __('An error occured.', 'mycategoryorder').'</p></div>';
	...
	if (isset($_POST['btnReturnParent'])) { 
		...
		$parentsParent = $wpdb->get_row($wpdb->prepare("SELECT parent FROM $wpdb->term_taxonomy WHERE term_id = %d ", $returnParentID) , ARRAY_N);
		$parentID = $parentsParent[0];
	}
	elseif (isset($_POST['btnOrderCats'])) { 
		check_admin_referer('update_mycategoryorder_'.$parentID);
...
<form name="frmMyCatOrder" method="post" action="">
	<?php wp_nonce_field('update_mycategoryorder_'.$parentID); ?>
...
function mycategoryorder_updateOrder()
{
	if (isset($_POST['hdnMyCategoryOrder']) && $_POST['hdnMyCategoryOrder'] != "") { 
		...
		for($i = 0; $i < $result; $i++)
		{
			...
			if(!is_numeric($str))
				return '<div id="message" class="error fade"><p>'. __('An error occured, order has not been saved.', 'mycategoryorder').'</p></div>';
